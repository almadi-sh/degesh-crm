...
export default function SupplierCards() {
  ...
  const tab = searchParams.get("tab") === "others" ? "others" : "suppliers";
  const pageTitle = tab === "others" ? "Прочие" : "Поставщики";
  const pageDescription = tab === "others"
    ? "Раздел объединяет список прочих контрагентов и карточки с товарами."
    : "Раздел объединяет список поставщиков и карточки с товарами.";
  ...
  const visibleSuppliers = useMemo(
    () => suppliers.filter((item) => (supplierKinds[String(item.id)] ?? "supplier") === (tab === "others" ? "other" : "supplier")),
    [suppliers, supplierKinds, tab],
  );
